...
import { Send, Sparkles, Trash2 } from "lucide-react"; // Import Sparkles and Trash2 icons
...
export function MailList({ items, category }: MailListProps) {
  ...
  const handleDeletePost = async (postId: string) => {
    if (window.confirm("Are you sure you want to delete this post and all its replies?")) {
      try {
        const success = await deletePostAndReplies(postId, category);
        if (success) {
          toast.success("Post and replies deleted successfully");
          // Update local state to remove the deleted post
          // You might need to implement this function or use a state management solution
          // removePostFromItems(postId);
        } else {
        ...
        <Badge variant="stone">
          <MagicWandIcon className="h-4 w-4 cursor-pointer " onClick={() => handleMagicPrompt(item.text, item.id)} />
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <IconWrapper>
                    <MessageSquare
                      className="toggle-reply-input"
                      data-tour="toggle-reply-input"
                      strokeWidth="1.5"
                      size="20"
